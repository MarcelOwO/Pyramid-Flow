...
model_path = os.path.join(
    current_directory, "pyramid_flow_model"
...
def download_model_from_hf(model_repo, model_dir, variants, required_file):
    ...
    if not os.path.exists(model_dir):
        print(
            f"[INFO] Model directory '{model_dir}' does not exist. Initiating download..."
        )
        need_download = True
    else:
        ...
        for variant_key, variant_dir in variants.items():
            ...
            if not os.path.exists(file_path):
                print(
                    f"[WARNING] Required file '{required_file}' missing in '{variant_path}'."
                )
                need_download = True
                break
    ...
    if need_download:
        ...
        print(
            f"[INFO] All required model files are present in '{model_dir}'. Skipping download."
        )
...
def initialize_model(variant):
    print(
        f"[INFO] Initializing model with variant='{variant}', using bf16 precision..."
    )
    ...
    if not os.path.exists(config_path):
        print(
            f"[ERROR] config.json not found in '{os.path.join(model_path, variant_dir)}'."
        )
        raise FileNotFoundError(
            f"config.json not found in '{os.path.join(model_path, variant_dir)}'."
        )
...
def generate_text_to_video(
    ...
    try:
        ...
        with (
            torch.no_grad(),
            torch.autocast(model.device.type, dtype=torch_dtype_selected),
        ):
            stage_outputs = model.generate(
                prompt=prompt,
            ...
            for stage_name, frames in stage_outputs.items():
                output_filename = f"pyramid_vis_{stage_name}.mp4"
                export_to_video(frames, output_filename, fps=24)
...
def generate_image_to_video(
    image, prompt, temp, video_guidance_scale, resolution, seed, progress=gr.Progress()
):
    progress(0, desc="Loading model")
        ...
        with gr.Row():
            with gr.Column():
                ...
                guidance_scale_slider = gr.Slider(
                    1.0, 15.0, value=9.0, step=0.1, label="Guidance Scale"
                )
                video_guidance_scale_slider = gr.Slider(
                    1.0, 10.0, value=5.0, step=0.1, label="Video Guidance Scale"
                )
                text_seed = gr.Number(
                    label="Inference Seed (Enter a positive number, 0 for random)",
                    value=0,
                )
    ...
    with gr.Tab("Image-to-Video"):
        with gr.Row():
            with gr.Column():
                ...
                image_video_guidance_scale_slider = gr.Slider(
                    1.0, 7.0, value=4.0, step=0.1, label="Video Guidance Scale"
                )
        ...
        gr.Examples(
            ...
            inputs=[
                ...
                image_prompt,
                image_temp_slider,
                image_video_guidance_scale_slider,
                resolution_dropdown,
                image_seed,
            ],
            outputs=[img_output],
