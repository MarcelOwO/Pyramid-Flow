...
def build_pyramid_dit(
    ...
    if model_name == "pyramid_flux":
        ...
        raise NotImplementedError(
            f"Unsupported DiT architecture, please set the model_name to `pyramid_flux` or `pyramid_mmdit`"
        )
...
def build_text_encoder(
    ...
    if load_text_encoder:
        if model_name == "pyramid_flux":
            ...
            raise NotImplementedError(
                f"Unsupported Text Encoder architecture, please set the model_name to `pyramid_flux` or `pyramid_mmdit`"
            )
    else:
        text_encoder = None
...
class PyramidDiTForVideoGeneration:
    ...
    def add_pyramid_noise(
        ...
        for i_s in range(stages - 1):
            ...
            cur_noise = (
                F.interpolate(cur_noise, size=(height, width), mode="bilinear") * 2
            )
        ...
        for index in range(column_size):
            ...
            last_clean_latent = (
                None if i_s == 0 else latents_list[i_s - 1][index::column_size]
            )
            ...
            if i_s == 0:
                ...
                last_clean_latent = rearrange(
                    last_clean_latent, "b c t h w -> (b t) c h w"
                )
                ...
                last_clean_latent = rearrange(
                    last_clean_latent, "(b t) c h w -> b c t h w", t=t
                )
                start_point = (
                    start_sigma * noise_list[i_s][index::column_size]
                    + (1 - start_sigma) * last_clean_latent
                )
            ...
            if i_s == stages - 1:
                ...
                end_point = (
                    end_sigma * noise_list[i_s][index::column_size]
                    + (1 - end_sigma) * clean_latent
                )
            ...
            indices = (
                u * training_steps
            ).long()  # Totally 1000 training steps per stage
            indices = indices.clamp(0, training_steps - 1)
            timesteps = self.scheduler.timesteps_per_stage[i_s][indices].to(
                device=device
            )
            ...
            last_cond_noisy_sigma = (
                torch.rand(size=(batch_size,), device=device) * self.corrupt_ratio
            )
    ...
    def sample_stage_length(self, num_stages, max_units=None):
        ...
        cur_highres_unit = max(
            int(
                (cur_rank % self.video_sync_group + 1)
                + update_turn * self.video_sync_group
            ),
            1,
        )
    ...
    def add_pyramid_noise_with_temporal_pyramid(
        ...
        if is_sequence_parallel_initialized():
            ...
            max_units_per_sample = all_to_all(
                max_units_per_sample.unsqueeze(1).repeat(1, sp_group_size),
                sp_group,
                sp_group_size,
                scatter_dim=1,
                gather_dim=0,
        ...
        num_units_per_stage = self.sample_stage_length(
            stages, max_units=max_units
        )  # [The unit number of each stage]
        ...
        for index in range(column_size):
            ...
            if num_units == 1:
                ...
                last_cond_latent = clean_latent[
                    :, :, -(2 * self.frame_per_unit) : -self.frame_per_unit
                ]
                ...
                last_cond_latent = (
                    last_cond_noisy_sigma * torch.randn_like(last_cond_latent)
                ...
                while cur_unit_num < num_units:
                    ...
                    if cur_stage == 0:
                        break
                    cur_unit_num += 1
                    cond_latents = latents_list[cur_stage][index::column_size][
                        :, :, :actual_frames
                    ]
                    cond_latents = cond_latents[
                        :,
                        :,
                        -(cur_unit_num * self.frame_per_unit) : -(
                            (cur_unit_num - 1) * self.frame_per_unit
                        ),
                    ]
                    cond_latents = (
                        last_cond_noisy_sigma * torch.randn_like(cond_latents)
                        + (1 - last_cond_noisy_sigma) * cond_latents
                    )
                    stage_input.append(cond_latents.to(dtype))

                if cur_stage == 0 and cur_unit_num < num_units:
                    cond_latents = latents_list[0][index::column_size][
                        :, :, :actual_frames
                    ]
                    cond_latents = cond_latents[
                        :, :, : -(cur_unit_num * self.frame_per_unit)
    ...
    def get_vae_latent(self, video, use_temporal_pyramid=True):
        if self.load_vae:
            assert (
                video.shape[1] == 3
            ), "The vae is loaded, the input should be raw pixels"
        ...
        if video.shape[2] == 1:
            ...
            # is video
            video[:, :, :1] = (
                video[:, :, :1] - self.vae_shift_factor
            ) * self.vae_scale_factor
            video[:, :, 1:] = (
                video[:, :, 1:] - self.vae_video_shift_factor
            ) * self.vae_video_scale_factor

        # Get the pyramidal stages
        vae_latent_list = self.get_pyramid_latent(video, len(self.stages) - 1)

        if use_temporal_pyramid:
            noisy_latents_list, ratios_list, timesteps_list, targets_list = (
                self.add_pyramid_noise_with_temporal_pyramid(
                    vae_latent_list, self.sample_ratios
                )
            )
        else:
            # Only use the spatial pyramidal (without temporal ar)
            noisy_latents_list, ratios_list, timesteps_list, targets_list = (
                self.add_pyramid_noise(vae_latent_list, self.sample_ratios)
            )
    ...
    def get_text_embeddings(self, text, rand_idx, device):
        if self.load_text_encoder:
            ...
            for idx in range(batch_size):
                if rand_idx[idx].item():
                    text["prompt_embeds"][idx] = self.null_text_embeds[
                        "prompt_embed"
                    ].to(device)
                    text["prompt_attention_mask"][idx] = self.null_text_embeds[
                        "prompt_attention_mask"
                    ].to(device)
                    text["pooled_prompt_embeds"][idx] = self.null_text_embeds[
                        "pooled_prompt_embed"
                    ].to(device)

            return (
                text["prompt_embeds"],
                text["prompt_attention_mask"],
    ...
    def calculate_loss(self, model_preds_list, targets_list):
        ...
        for model_pred, target in zip(model_preds_list, targets_list):
            ...
            loss = torch.mean(
                (
                    loss_weight.float() * (model_pred.float() - target.float()) ** 2
    ...
    def __call__(
        ...
        with torch.no_grad(), accelerator.autocast():
            ...
            prompt_embeds, prompt_attention_mask, pooled_prompt_embeds = (
                self.get_text_embeddings(text, rand_idx, device)
            )
            noisy_latents_list, ratios_list, timesteps_list, targets_list = (
                self.get_vae_latent(video, use_temporal_pyramid=use_temporal_pyramid)
            )

        timesteps = torch.cat(
            [timestep.unsqueeze(-1) for timestep in timesteps_list], dim=-1
        )
    ...
    def sample_block_noise(self, bs, ch, temp, height, width):
        ...
        dist = torch.distributions.multivariate_normal.MultivariateNormal(
            torch.zeros(4),
            torch.eye(4) * (1 + gamma) - torch.ones(4, 4) * gamma + epsilon,
        )
    ...
    def generate_one_unit(
        ...
        stages = self.stages
        # intermed_latents = []
        # Dictionary to store output latents at different scales
        stage_latents = {}
        ...
        for i_s in range(len(stages)):
            ...
            # store latent inside each loop run at the end
            stage_latents[i_s] = latents.clone()
            # intermed_latents.append(latents)
        return stage_latents
    ...
    def generate_i2v(
        ...
        ) = self.text_encoder(negative_prompt, device)
        ...
        num_channels_latents = (
            (self.dit.config.in_channels // 4)
            if self.model_name == "pyramid_flux"
            else self.dit.config.in_channels
        )
    ...
    def generate(
        ...
        stage_latents_history = {i_s: [] for i_s in range(len(stages))}
        ...
        for unit_index in tqdm(range(num_units)):
            ...
            if unit_index == 0:
                ...
                stage_latents_dict = self.generate_one_unit(
                    latents[:, :, :1],
                    past_condition_latents,
                    prompt_embeds,
                    prompt_attention_mask,
                ...
                clean_latents_list = self.get_pyramid_latent(
                    # torch.cat(generated_latents_list, dim=2), len(stages) - 1
                    torch.cat(stage_latents_history[len(stages) - 1], dim=2),
                    len(stages) - 1,
                )
            ...
            for i_s, stage_tensor in stage_latents_dict.items():
                stage_latents_history[i_s].append(stage_tensor)

            # generated_latents_list.append(intermed_latents[-1])
            # last_generated_latents = intermed_latents

            generated_latents_list.append(stage_latents_dict[len(stages) - 1])
            last_generated_latents = stage_latents_dict
            # torch.cat(generated_latents_list, dim=2)

        if output_type == "latent":
            return {
                f"stage_{i_s}": torch.cat(stage_latents_history[i_s], dim=2)
                for i_s in range(len(stages))
            }
            ...
            stage_outputs = {}
            for i_s in range(len(stages)):
                full_stage_latent = torch.cat(stage_latents_history[i_s], dim=2)
                decoded_frames = self.decode_latent(
                    full_stage_latent,
                    save_memory=save_memory,
                    inference_multigpu=inference_multigpu,
                )
                stage_outputs[f"stage_{i_s}"] = decoded_frames
